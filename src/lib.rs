...
pub unsafe extern "C" fn Reset() -> ! {
    ...
    // Assign the VTOR value where the vector table really is, by default it
    // seems to be set to 0x00200000 on the imxrt1062
    // this is not where the vector table really.
    //
    // Startup works thanks to either loading with a debug probe or booting
    // from flash with the imxrt bootloader which allows an invalid VTOR
    // to start.
    const VTOR: *mut u32 = 0xE000ED08 as *mut u32;
    let vtor: u32 = &__vector_table as *const u32 as u32;
    core::ptr::write_volatile(VTOR, vtor);

    // Disable watchdogs
    //
    // WDOG1 and WDOG2 by default have 16 seconds before they effectively
    // put the chip to sleep if they aren't disabled. Without this the chip
    // falls asleep.
    //
    // RTWDOG (WDOG3) is a safety watchdog enabled by default and locked.
    // After unlocking RTWDOG using the key provided in the reference manual
    // 255 cycles are allowed to reconfigure it. By default we turn it off
    // here.
    const WDOG1_WCR: *mut u16 = (0x400B8000 + 0x0) as *mut u16;
    const WDOG1_WCMR: *mut u16 = (0x400B8000 + 0x8) as *mut u16;
    const WDOG2_WCR: *mut u16 = (0x400D0000 + 0x0) as *mut u16;
    const WDOG2_WCMR: *mut u16 = (0x400D0000 + 0x8) as *mut u16;
    const WDOG_WCR_WDE_MASK: u16 = 0x4;
    const WDOG_WMCR_PDE_MASK: u16 = 0x1;

    const RTWDOG_BASE: u32 = 0x400BC000;
    const RTWDOG_CS: *mut u32 = (RTWDOG_BASE + 0x0) as *mut u32;
    const RTWDOG_CNT: *mut u32 = (RTWDOG_BASE + 0x4) as *mut u32;
    const RTWDOG_TOVAL: *mut u32 = (RTWDOG_BASE + 0x8) as *mut u32;
    const RTWDOG_CS_CMD32EN_MASK: u32 = 0x2000;
    const RTWDOG_CS_EN_MASK: u32 = 0x80;
    const RTWDOG_CS_UPDATE_MASK: u32 = 0x20;
    let wdog1_wmcr = core::ptr::read_volatile(WDOG1_WCMR);
    core::ptr::write_volatile(WDOG1_WCMR, wdog1_wmcr & !WDOG_WMCR_PDE_MASK);
    let wdog2_wmcr = core::ptr::read_volatile(WDOG2_WCMR);
    core::ptr::write_volatile(WDOG2_WCMR, wdog2_wmcr & !WDOG_WMCR_PDE_MASK);
    let wdog1_wcr = core::ptr::read_volatile(WDOG1_WCR);
    if (wdog1_wcr & WDOG_WCR_WDE_MASK) != 0 {
        core::ptr::write_volatile(WDOG1_WCR, wdog1_wcr & !WDOG_WCR_WDE_MASK);
    }
    let wdog2_wcr = core::ptr::read_volatile(WDOG2_WCR);
    if (wdog2_wcr & WDOG_WCR_WDE_MASK) != 0 {
        core::ptr::write_volatile(WDOG2_WCR, wdog2_wcr & !WDOG_WCR_WDE_MASK);
    }
    let rtwdog_cs = core::ptr::read_volatile(RTWDOG_CS);
    if (rtwdog_cs & RTWDOG_CS_CMD32EN_MASK) != 0 {
        // 0xD928C520U
        core::ptr::write_volatile(RTWDOG_CNT, 0xD928C520);
    } else {
        core::ptr::write_volatile(RTWDOG_CNT, 0xC520);
        core::ptr::write_volatile(RTWDOG_CNT, 0xD928);
    }
    core::ptr::write_volatile(RTWDOG_TOVAL, 0xFFFF);
    core::ptr::write_volatile(
        RTWDOG_CS,
        (rtwdog_cs & !RTWDOG_CS_EN_MASK) | RTWDOG_CS_UPDATE_MASK,
    );

    // TODO Disable SysTick
    //const SYSTICK_CTRL: *mut u32 =
